...
public class Enemigos {
	...
	public void añadirTrollsAmarillos(int num, Dimension dim,
			boolean playState, double velocity) {
		int check = num;
		while (check > 0) {
			this.mobs
					.add(new TrollAmarillo(dim, playState, (velocity * 2) / 3));
			check = check - 1;
		}
	}

	public void añadirTrollsRojos(int num, Dimension dim, boolean playState,
			double velocity) {
		int check = num;
		while (check > 0) {
	...
	public void añadirTrollsRojosEnojados(int num, Dimension dim,
			boolean playState, double velocity) {
		int check = num;
		while (check > 0) {
			this.mobs.add(new TrollRojoEnojado(dim, playState,
					(velocity * 2) / 3));
			check = check - 1;
		}
	}

	public void añadirTrollsVerdes(int num, Dimension dim, boolean playState,
			double velocity) {
		int check = num;
		while (check > 0) {
	...
	public Enemigos enemigosParaBoss(Dimension dim, boolean playState,
			double velocity) {
