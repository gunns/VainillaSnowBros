...
public class SnowBrosScene extends GameScene {
	...
	public SnowBrosScene(Dimension dim, double velocity) throws Exception {
		...
		this.backGround = new GameComponent<GameScene>(
				sprite.scale(0.46, 0.57), 0, 0);
	...
	public void comenzarJuego() {
		...
		CartelInicioNivel cartelInicio = new CartelInicioNivel(gameDimension,
				numeroNivel, velocity);
	...
	private void buildBackground(Color color) {
		...
		this.backGround = new GameComponent<GameScene>(sprite.scale(
				name.getEscala1(), name.getEscala2()), 0, 0);
		this.backGround.setZ(-1);
		this.addComponent(this.backGround);
	}
	...
	public boolean hayColisionConUnPiso(
			GameComponent<SnowBrosScene> componenteRectangular) {
		...
		for (GameComponent<?> each : this.getComponents())
			if (each.getClass() == Piso.class) {
				...
				if (CollisionDetector.INSTANCE
						.collidesRectAgainstRect(componenteRectangular.getX(),
								(componenteRectangular.getY()
										+ componenteRectangular.getAppearance()
												.getHeight() - 1),
								(int) (componenteRectangular.getAppearance()
										.getWidth()), (int) (1), unPiso.getX(),
								unPiso.getY(), (int) (unPiso.getAppearance()
										.getWidth()), (int) (2))) {
					hayColision = true;
				}
			}
	...
	public boolean hayColisionTotalConUnPiso(
			GameComponent<SnowBrosScene> componenteRectangular) {
		boolean hayColision = false;
		for (GameComponent<?> each : this.getComponents())
			if (each.getClass() == Piso.class) {
				...
				if (CollisionDetector.INSTANCE
						.collidesRectAgainstRect(componenteRectangular.getX(),
								...
								(int) (componenteRectangular.getAppearance()
										.getWidth()),
								(int) (componenteRectangular.getAppearance()
										.getHeight()), unPiso.getX(), unPiso
										.getY(), (int) (unPiso.getAppearance()
										.getWidth()), (int) (unPiso
	...
	public boolean enemigoColisionaConBros(
			GameComponent<SnowBrosScene> componenteRectangular) {
		boolean hayColision = false;
		Mob mob;
		for (int n = 0; n < this.enemigos.getEnemigos().size(); n++) {
			...
			if (CollisionDetector.INSTANCE.collidesRectAgainstRect(
					componenteRectangular.getX(), componenteRectangular.getY(),
					(int) componenteRectangular.getAppearance().getWidth(),
					(int) componenteRectangular.getAppearance().getHeight(),
					mob.getX(), mob.getY(), (int) mob.getAppearance()
							.getWidth(), (int) mob.getAppearance().getHeight())) {
		...
		for (GameComponent<?> each : this.getComponents()) {
			if (esUnMob(each)) {
				...
				if (esfera.getEstadoNieve().PuedoEmpujar()) {
					if (CollisionDetector.INSTANCE.collidesRectAgainstRect(
							esfera.getX(), esfera.getY(), (int) esfera
									.getAppearance().getWidth(), (int) esfera
									.getAppearance().getHeight(), bros.getX(),
	...
	private boolean colisionNieveConMob(GameComponent<?> each, Mob mob2) {
		...
		if (CollisionDetector.INSTANCE.collidesCircleAgainstRect(nieve.getX(),
				nieve.getY(), nieve.getAppearance().getWidth() / 2,
				mob2.getX(), mob2.getY(), mob2.getAppearance().getWidth(), mob2
						.getAppearance().getHeight())) {
			hayColision = true;
			each.destroy();
		}
	...
	public boolean puedeEmpujarBolaDeNieve(Bros bros, DeltaState deltaState) {
		...
		for (GameComponent<?> each : c) {
			if (esUnMob(each)) {
				Mob mob = (Mob) each;
				if (this.colisionBolaNieveConBros(each, bros)
						&& mob.getEstadoNieve().PuedoEmpujar()) {
					puedeEmpujar = true;
				}
			}
		}
	...
	public boolean colisionBolaNieveConBros(GameComponent<?> each, Bros bros) {
		...
		if (CollisionDetector.INSTANCE.collidesCircleAgainstRect(mob.getX(),
				mob.getY(), mob.getAppearance().getWidth() / 2, bros.getX(),
				bros.getY(), bros.getAppearance().getWidth(), bros
						.getAppearance().getHeight())) {
			hayColision = true;
	...
	public boolean colisionBolaNieveFConBros(Bros bros, Mob mob) {
		return (CollisionDetector.INSTANCE.collidesRectAgainstRect(bros.getX(),
				bros.getY(), (int) (bros.getAppearance().getWidth()),
				(int) (bros.getAppearance().getHeight()), mob.getX(),
				mob.getY(), (int) (mob.getAppearance().getWidth()),
				(int) (mob.getAppearance().getHeight())));
	}
	...
	public boolean colisionBolaRodanteConBros(GameComponent<?> each, Bros bros) {
		...
		GameComponent<?> mob = each;
		if (CollisionDetector.INSTANCE.collidesCircleAgainstRect(mob.getX(),
				mob.getY(), mob.getAppearance().getWidth() / 3, bros.getX(),
				bros.getY(), bros.getAppearance().getWidth(), bros
						.getAppearance().getHeight())) {
			hayColision = true;
		...
		for (GameComponent<?> each : c) {
			if (esUnMob(each)) {
				...
				if (this.colisionBolaNieveConBros(mob, bros)
						&& mob.getEstadoNieve().PuedoEmpujar()
						&& this.hayColisionConUnPiso(bros)) {
	...
	public boolean esferaRodanteColisionaConBros(Bros bros) {
		...
		for (GameComponent<?> each : esferas) {
			if (esUnMob(each)) {
				...
				if (this.colisionBolaRodanteConBros(each, bros)
						&& mob.getEstadoNieve().puedeRebotar()) {
					colisiona = true;
				}
			}
		}
	...
	public boolean colisionaEsferaConMob(Mob mob, Mob mobColisionado) {
		boolean hayColision = false;
		if (CollisionDetector.INSTANCE.collidesCircleAgainstRect(mob.getX(),
				mob.getY(), mob.getAppearance().getWidth() / 2, mobColisionado
						.getX(), mobColisionado.getY(), mobColisionado
						.getAppearance().getWidth(), mobColisionado
						.getAppearance().getHeight())) {
			hayColision = true;
	...
	public boolean colisionaEsferaConEsfera(Mob mob, Mob mob2) {
		if (CollisionDetector.INSTANCE.collidesCircleAgainstCircle(mob.getX(),
				mob.getY(), (int) mob.getAppearance().getWidth() / 2,
				mob2.getX(), mob2.getY(),
				(int) mob2.getAppearance().getWidth() / 2)) {
			return true;
		} else
			return false;
	}
	...
	private boolean esUnTesoro(GameComponent<?> each) {
		...
		return (each.getClass() == Caramelo.class
				...
				|| each.getClass() == Caramelo.class || each.getClass() == Capsula.class);
	}
	...
	public boolean tieneUnPisoJustoArriba(double x, double y, int height,
			int width) {
		...
		rectanguloPrueba
				.setAppearance(new Rectangle(Color.blue, width, height));
	...
	public void gameOverMusic() {
		...
		Sound sonidoGameOver = new SoundBuilder().buildSound(this.getClass()
				.getClassLoader().getResourceAsStream("gameOver.wav"));
		sonidoGameOver.play();
	...
	public boolean enemigosExterminados() {
		...
		for (GameComponent<?> each : this.getComponents()) {
			...
			} else if (esUnMob(each) || esUnTesoro(each)) {
				nivelTerminado = false;
			}
		}
	...
	public void nivelCompleto() {
		...
		CartelLevelComplete cartel = new CartelLevelComplete(gameDimension,
				this.numeroNivel);
		this.addComponent(cartel);
		Sound sonidoNivelCompleto = new SoundBuilder().buildSound(this
				.getClass().getClassLoader()
				.getResourceAsStream("nextLevel.wav"));
	...
	public void siguienteNivel() {
		if (this.numeroNivel == 15) {
			...
			this.backGround = new GameComponent<GameScene>(nivelBoss
					.getImagenFondo().scale(0.9, 1.2), 0, 0);
		...
		CartelSiguienteNivel cartel = new CartelSiguienteNivel(gameDimension,
				this.numeroNivel);
		this.addComponent(cartel);
	...
	public boolean brosEstaAbajoDeMob(Mob mob) {
		...
		for (GameComponent<?> each : this.getComponents()) {
			if (each.getClass() == Bros.class) {
				Bros bros = (Bros) each;
				estaAbajo = (mob.getY() - mob.getAppearance().getHeight()) < (bros
						.getY() - bros.getAppearance().getHeight());
			}
		}
		return estaAbajo;
	}
	...
	public boolean brosEstaALaAlturaDelMob(Mob mob) {
		...
		for (GameComponent<?> each : this.getComponents()) {
			if (each.getClass() == Bros.class) {
				Bros bros = (Bros) each;
				estaIgual = ((bros.getY() + 20) > (mob.getY()) && (bros.getY() - 20) < mob
						.getY());
			}
		}
		return estaIgual;
	...
	public boolean brosEstaArribaDeMob(Mob mob) {
		...
		for (GameComponent<?> each : this.getComponents()) {
			if (each.getClass() == Bros.class) {
				Bros bros = (Bros) each;
				estaArriba = (mob.getY() - mob.getAppearance().getHeight()) > (bros
						.getY() - bros.getAppearance().getHeight());
			}
		}
		return estaArriba;
	...
	public boolean esUnMob(GameComponent<?> element) {
		return (element.getClass() == Mob.class
				...
				|| element.getClass() == TrollRojo.class || element.getClass() == TrollRojoEnojado.class);
	}
		...
		for (GameComponent<?> each : this.getComponents()) {
			if (each.getClass() == Bros.class) {
				...
				if (!bros.muriendo) {
					if (CollisionDetector.INSTANCE.collidesRectAgainstRect(
							bros.getX(), bros.getY(),
							(int) (bros.getAppearance().getWidth()),
							(int) (bros.getAppearance().getHeight()),
							disparo.getX(), disparo.getY(),
							(int) (disparo.getAppearance().getWidth()),
							(int) (disparo.getAppearance().getHeight()))
							&& !bros.invencible) {
						bros.matarBros();
					}
				}
	...
	public boolean tocoFondo(Boss boss) {
		return ((boss.getY() + boss.getAppearance().getHeight()) > (this
				.getGameDimension().getHeight()) - 10);
	}
	...
	public void invocarEnemigos() {
		this.enemigos = this.enemigos.enemigosParaBoss(gameDimension,
				playState, velocity);
		this.addComponents(this.enemigos.getEnemigos());
	}
		...
		for (GameComponent<?> each : esferas) {
			if (esUnMob(each)) {
				...
				if (mob.getEstadoNieve().puedeRebotar()) {
					if (CollisionDetector.INSTANCE.collidesRectAgainstRect(
							boss.getX(), boss.getY(),
							(int) (boss.getAppearance().getWidth()),
							(int) (boss.getAppearance().getHeight()),
							mob.getX(), mob.getY(),
							(int) (mob.getAppearance().getWidth()), (int) (2))) {
						colisiona = true;
					}
				}
			}
